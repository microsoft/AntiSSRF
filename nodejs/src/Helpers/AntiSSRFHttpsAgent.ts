...
/**
 * Any changes in this file need to be copied into AntiSSRFHttpAgent.ts as well.
 * The two files must always be the same, except that this one extends
 * https.Agent while AntiSSRFHttpAgent extends http.Agent.
 *
...
export class AntiSSRFHttpsAgent extends HttpsAgent {
    private _antiSSRFPolicy: AntiSSRFPolicy;
    private _antiSSRFLookup: LookupFunction;

    constructor(policy: AntiSSRFPolicy, options?: HttpsAgentOptions) {
        // Ensure the user does not expect to use a different dns.lookup function
        if (options?.lookup != null) {
            throw new AntiSSRFError("Cannot use AntiSSRFHttpsAgent with custom lookup function");
        }
    ...
    /**
     * This function is a wrapper around the addAgent function in https.Agent.
     ...
     * https.Agent. It is used to check if the host is an IP address, and if so,
     ...
     * https.Agent first checks if isIP(host), and if so, it skips our
     ...
     * if the host is an allowed IP address before letting the https.Agent
